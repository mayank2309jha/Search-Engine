...
**Update (Phase 14): pushed, and CI's first real run on GitHub caught a second, different real bug** — the fixture fix above wasn't the whole story. A fresh runner has neither the sentence-transformer model nor NLTK's stopwords corpus cached (every local run, including the reproduction above, already had both cached from prior, unrelated use); `python-tests` failed with a collection-time error (exit code 2) as a result. Fixed by giving the plain `pytest` job the same pre-download step the Docker image already had (Phase 12), verified against an isolated empty-`HOME` reproduction before pushing again. All 5 jobs are now green on GitHub — see `runningUpdates.md` Phase 14.
